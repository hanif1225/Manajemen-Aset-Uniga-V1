...
<a href="#" class="btn btn-primary" data-toggle="modal" data-target="#add">Tambah Inventarisir</a>
...
<td>&nbsp Dari Tanggal</td>
    <td><input type="date" name="tgl_awal" required=""></td>
    <td>Sampai Tanggal</td>
    <td><input type="date" name="tgl_akhir" required=""></td>
<td>
...
<form method="post" enctype="multipart/form-data" class="form-inline" id="pencarian_id"  action="<?php echo $_SERVER['PHP_SELF']?> ">
  <div class="form-group mx-sm-1 mb-1">
    ...
    <input type="date" class="form-control" name="keyword2"  placeholder="Masukkan Tanggal">
  </div>
...
<!-- <form method="post" enctype="multipart/form-data" class="form-inline" id="pencarian_id"  action="<?php echo $_SERVER['PHP_SELF']?> ">
  ...
  <button name="pencarian2" type="submit" class="btn btn-primary mb-2">Cari Data Berdasarkan Tanggal</button>
</form> -->
    ...
    <table class="table table-success table-striped">
        <br>
            ...
            if(isset($_POST['pencarian'])){
                ...
                elseif($keyword == '' &&  $keyword2 != '')
                {
                  $sql = "SELECT * from inventaris INNER JOIN nama_barang on nama_barang.id_barang=inventaris.id_barang
                 INNER JOIN nama_lokasi on nama_lokasi.id_lokasi=inventaris.id_lokasi  
                 where tgl_pengadaan like '%$keyword2%' order by tgl_pengadaan ASC limit $mulai,$halaman";
                }
                elseif($keyword != '' &&  $keyword2 != '')
                {
                  $sql = "SELECT * from inventaris INNER JOIN nama_barang on nama_barang.id_barang=inventaris.id_barang
                  INNER JOIN nama_lokasi on nama_lokasi.id_lokasi=inventaris.id_lokasi  
                  where nm_barang like '%$keyword%' AND  tgl_pengadaan like '%$keyword2%' order by nm_barang ASC limit $mulai,$halaman";
                }

            }
            // else if(isset($_POST['pencarian2'])){
            //     //query pencarian data
            //     $keyword2=$_POST['keyword2'];
            //     $sql = "SELECT * from inventaris INNER JOIN nama_barang on nama_barang.id_barang=inventaris.id_barang
            //     INNER JOIN nama_lokasi on nama_lokasi.id_lokasi=inventaris.id_lokasi  
            ...
            // }
              ...
              echo "<script>window.location='export.php?keadaan=$keadaan&tgl_awal=$tgl_awal&tgl_akhir=$tgl_akhir'</script>";
            }
            else{
            ...
            ?>
                <tr>
                    ...
                    <td>
                    <a href=<?php echo "'proses_use.php?id_inventaris=$row[id_inventaris]'" ?>  class='btn btn-primary btn-sm'>
                    Digunakan
                    </a> 
                    <a href=<?php echo "'edit.php?id_inventaris=$row[id_inventaris]'" ?>  class='btn btn-warning btn-sm'>
                    <i class='fas fa-edit'></i></a>
                    <a href=<?php echo "'proses_rusak.php?id_inventaris=$row[id_inventaris]'" ?>  class='btn btn-primary btn-sm'>
                    <i class="fas fa-toolbox"></i>
                    </a>
                    <a href=<?php echo "'detail.php?id_inventaris=$row[id_inventaris]'" ?>  class='btn btn-success btn-sm'><i class="fas fa-search"></i></a>
                    <a onclick="return confirm('Apakah data ingin dihapus')"  class='btn btn-danger btn-sm' href=<?php echo "'delete.php?id_inventaris=$row[id_inventaris]'"?>  ><i class='fas fa-trash-alt'></i></a>
                    </td>
                </tr>
